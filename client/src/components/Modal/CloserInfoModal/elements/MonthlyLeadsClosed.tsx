...
import { getUserYearlyLeadsClosed } from "../../../../api/user";
import { returnDarkColors } from "../../../../utils/utils";

const MonthlyLeadsClosed = ({ userId }: { userId: number }) => {
    const { data = [] } = useQuery({
        queryKey: [userId, "month-wise-leads-closed"],
        queryFn: () => getUserYearlyLeadsClosed(userId),
    });
    ...
    const events = data?.map((item: any) => ({
        ...
        subtitle: (
            <span className="mt-1 block">Lead By: {item?.leadBy?.alias}</span>
        ),
...
export default MonthlyLeadsClosed;
